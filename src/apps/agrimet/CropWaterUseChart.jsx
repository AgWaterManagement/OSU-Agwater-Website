...
const CropWaterUseChart = ({ selectedStation, cropETData }) => {
    ...
    const yesterdayMonth = _yesterday.getMonth() + 1;
    const yesterdayDay = _yesterday.getDate();

    return (
        <div style={{ marginLeft: '1em' }}>

            {cropETData && cropETData.data ? (
                <div style={{ margin: 0 }}>
            ...
            ) : cropETData.data && Object.keys(cropETData.data).length > 0 ? (
                <div style={{ marginLeft: '1em' }}>Crop water use data could not be fetched.</div>
            ) : (
                  <div style={{ marginLeft: '1em' }}>No crop water use data available.</div>
            )}
            ...
            <span style={{ marginLeft: 8, fontSize: 12, color: '#BBB' }}>
                ...
                <a style={{ color: '#93c6f5' }}
                    ...
                    {`https://agwater.org:5556/agrimet/station/daily_et?station=${selectedStation}`}
                </a>;
            </span>
            <br/>
             ...
             <span style={{ marginLeft: 8, fontSize: 12, color: '#BBB' }}>
                Agrimet Direct Link: 
                    <a style={{ color: '#93c6f5' }}
                    href={`https://www.usbr.gov/pn-bin/daily.pl?station=${selectedStation}&year=${currentYear}&month=1&day=1&year=${currentYear}&month=${yesterdayMonth}&day=${yesterdayDay}&pcode=ET&pcode=ETRS&pcode=ETOS&pcode=MN&pcode=MX&pcode=PP`}
                    target="_blank" rel="noopener noreferrer">
                    {`https://www.usbr.gov/pn-bin/daily.pl?station=${selectedStation}&year=${currentYear}&month=1&day=1&year=${currentYear}&month=${yesterdayMonth}&day=${yesterdayDay}&pcode=ET&pcode=ETRS&pcode=ETOS&pcode=MN&pcode=MX&pcode=PP`}
                </a>;
            </span>
